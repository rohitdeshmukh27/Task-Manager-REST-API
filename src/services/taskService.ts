...
export const getAllTasks = async (
    params: TaskQueryParams = {}): Promise<{data:Task[]|null;error:any;count:number|null}>=>{
    ...
    export const updateTask = async (
        ...
        const updateDate: Partial<Task>={};

        if(updates.title !== undefined) updateDate.title = updates.title;
        if(updates.description !== undefined) updateDate.description = updates.description;
        if(updates.status!== undefined) updateDate.priority = updates.priority;
        if(updates.due_date!==undefined) updateDate.due_date= updates.due_date;
